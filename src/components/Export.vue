    ...
    <div class="overlay">
        <div id="export-box">
            ...
            <div class="export-option input">
                ...
                <input type="number" min="1" max="60" v-model.number="store.options.framerate" id="export-framerate">
                <input type="text" id="export-filename" v-model="store.options.filename" placeholder="output"
                    spellcheck="false">
                <span class="path-select">
                    <input type="text" id="export-path" v-model="store.options.path"
                        :placeholder="$t('export.canNotBeEmpty')" spellcheck="false">
            ...
            <span class="export-progress-show">
                <span class="export-progress-wrapper">
                    <progress id="export-progress" :value="store.progress.current" :max="store.progress.total">
                    </progress>
                </span>
...
if (ipc) {
    const selectExportPath = async () => {
        store.options.path = await ipc.invoke('select-export-path')
    }
}
